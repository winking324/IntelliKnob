...
static KnobConfig configs[] = {
    ...
    },
    {
        ...
        1,
        1,
        0.6,  // Note the snap point is slightly past the midpoint (0.5);
  ...
  while (1) {
    ...
    if (scale.wait_ready_timeout(100)) {
      ...
      if (reading >= lower - (upper - lower) &&
        ...
        long value = std::clamp(reading, lower, upper);
    ...
    // TODO: brightness scale factor should be configurable (depends on
    // reflectivity of surface)

    brightness =
        (uint16_t)std::clamp(lux_avg * 13000, (float)1280, (float)UINT16_MAX);
